...
def collect(database: Path, day: date):
    start = datetime.combine(day, daytime(), TZ).astimezone(timezone.utc)
...
def send(webhook: str, text: str):
    ...
    for attempt in range(4):
        if attempt:
            time.sleep(2 ** attempt)  # 2s / 4s / 8s 退避，避开飞书租户级限流
        try:
            ...
            if code == 0:
                return result
            last_error = RuntimeError(f'Feishu rejected report: code={code}')
            if code != 11232:  # 11232 = frequency limited；其余错误重试没有意义
                raise last_error
